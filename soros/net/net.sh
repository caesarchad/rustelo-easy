...
Operate a configured testnet
 ...
 start/update-specific options:
   ...
   -t edge|beta|stable|vX.Y.Z         - Deploy the latest tarball release for the
                                        ...
                                        (vX.Y.Z)
   -i update_manifest_keypair         - Deploy the tarball using 'soros-install deploy ...'
...
build() {
  ...
  if [[ $(uname) != Linux || ! " ${supported[*]} " =~ $(lsb_release -sr) ]]; then
    ...
    source "$SOROS_ROOT"/ci/rust-version.sh
...
startCommon() {
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "soros@$ipAddress"
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./soros/net/remote/remote-client.sh $deployMethod $entrypointIp \
  ...
  if [[ -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./soros/net/remote/remote-sanity.sh $sanityExtraArgs -o noLedgerVerify -o noValidatorSanity \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  echo "--- Deploying soros-install update: $updateDownloadUrl"
  (
    set -x
    timeout 30s scp "${sshOptions[@]}" \
      "$updateManifestKeypairFile" "$bootstrapLeader:soros/update_manifest_keypair.json"

    # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./soros/net/remote/remote-deploy-update.sh $updateDownloadUrl \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
}
...
start() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      updateDownloadUrl=http://release.soros.com/"$releaseChannel"/soros-release-x86_64-unknown-linux-gnu.tar.bz2
      (
        set -x
        curl -o "$SOROS_ROOT"/soros-release.tar.bz2 "$updateDownloadUrl"
      )
  ...
  *)
  ...
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "soros-bench-tps"
    else
      startClient "${clientIpList[$i]}" "soros-bench-exchange"
    fi
  done
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$SOROS_ROOT"/soros-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
