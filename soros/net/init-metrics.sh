...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and BITCONCH_METRICS_CONFIG is
...
if $useEnv; then
  [[ -n $BITCONCH_METRICS_CONFIG ]] ||
    usage "BITCONCH_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  BITCONCH_METRICS_CONFIG="db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export BITCONCH_METRICS_CONFIG=\"$BITCONCH_METRICS_CONFIG\"" >> "$configFile"
