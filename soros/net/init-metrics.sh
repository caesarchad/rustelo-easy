...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and SOROS_METRICS_CONFIG is
...
if $useEnv; then
  [[ -n $SOROS_METRICS_CONFIG ]] ||
    usage "SOROS_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  SOROS_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export SOROS_METRICS_CONFIG=\"$SOROS_METRICS_CONFIG\"" >> "$configFile"
