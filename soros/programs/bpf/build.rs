...
fn main() {
    ...
    if bpf_rust {
        ...
        if !Path::new("rust/noop/target/bpfel-unknown-unknown/release/soros_bpf_rust_noop.so")
            .is_file()
        {
            ...
            panic!(
                "soros_bpf_rust_noop.so not found, you must manually run \
        ...
        rerun_if_changed(
            &[
                ...
                "rust/noop/target/bpfel-unknown-unknown/release/soros_bpf_rust_noop.so",
            ],
            &["rust/noop/src"],
        );

        println!(
            "cargo:warning=(not a warning) Installing Rust-based BPF program: soros_bpf_rust_noop"
        );
        ...
        let status = Command::new("cp")
            .current_dir("rust/noop")
            .arg("target/bpfel-unknown-unknown/release/soros_bpf_rust_noop.so")
            .arg(&install_dir)
            .status()
            .expect("Failed to copy soros_rust_bpf_noop.so to install directory");
        assert!(status.success());
    }
}
