...
use soros_runtime::bank::{Bank, Result};
...
use soros_sdk::transaction_builder::{BuilderInstruction, TransactionBuilder};
use soros_vote_api::vote_instruction::{Vote, VoteInstruction};
...
impl<'a> VoteBank<'a> {
    fn new(bank: &'a Bank) -> Self {
        bank.add_native_program("soros_vote_program", &soros_vote_api::id());
        Self { bank }
    }
