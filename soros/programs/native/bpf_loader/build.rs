...
fn main() {
    ...
    if bpf_rust {
        ...
        if !Path::new("../../bpf/rust/noop/out/bitconch_bpf_rust_noop.so").is_file() {
            ...
            panic!(
                "bitconch_bpf_rust_noop.so not found, you must manually run \
        ...
        rerun_if_changed(
            &[
                ...
                "../../bpf/rust/noop/out/bitconch_bpf_rust_noop.so",
            ],
            &[],
        );

        println!(
            "cargo:warning=(not a warning) Installing Rust-based BPF program: bitconch_bpf_rust_noop"
        );
        let status = Command::new("make")
