...
use log::*;
use soros_rewards_api::rewards_instruction::RewardsInstruction;
...
// https://github.com/bitconch/bus/blob/master/book/src/ed_vce_state_validation_protocol_based_rewards.md
// https://github.com/bitconch/bus/blob/master/book/src/staking-rewards.md
...
fn redeem_vote_credits(keyed_accounts: &mut [KeyedAccount]) -> Result<(), ProgramError> {
    ...
    if !soros_vote_api::check_id(&keyed_accounts[0].account.owner) {
...
mod tests {
    use super::*;
    use soros_rewards_api;
    use soros_rewards_api::rewards_state::RewardsState;
    ...
    fn create_rewards_account(lamports: u64) -> Account {
        ...
        Account::new(lamports, space, &soros_rewards_api::id())
    }
