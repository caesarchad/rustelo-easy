...
use soros_config_api::config_instruction;
use soros_sdk::pubkey::Pubkey;
use soros_sdk::signature::{read_keypair, Keypair, KeypairUtil, Signable};
...
fn load_release_target(release_dir: &Path) -> Result<String, Box<dyn std::error::Error>> {
    ...
    version_yml.push("soros-release");
...
fn check_env_path_for_bin_dir(config: &Config) {
    ...
    if !found {
        println!(
            "\nPlease update your PATH environment variable to include the soros programs:\n    PATH=\"{}:$PATH\"\n",
...
pub fn update(config_file: &str) -> Result<bool, String> {
    ...
    std::os::unix::fs::symlink(
        release_dir.join("soros-release"),
