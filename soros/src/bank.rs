...
use log::Level;
use rayon::prelude::*;
use bitconch_runtime::{self, RuntimeError};
use bitconch_sdk::account::Account;
use bitconch_sdk::bpf_loader;
use bitconch_sdk::budget_program;
...
use bitconch_sdk::timing::duration_as_us;
use bitconch_sdk::token_program;
use bitconch_sdk::transaction::Transaction;
use bitconch_sdk::vote_program::{self, VoteState};
use std;
use std::result;
...
impl Bank {
    ...
    fn add_builtin_programs(&self) {
        let system_program_account = native_loader::create_program_account("bitconch_system_program");
        self.accounts
            .store_slow(true, &system_program::id(), &system_program_account);

        let vote_program_account = native_loader::create_program_account("bitconch_vote_program");
        self.accounts
            .store_slow(true, &vote_program::id(), &vote_program_account);

        let storage_program_account =
            native_loader::create_program_account("bitconch_storage_program");
        self.accounts
        ...
        let bpf_loader_account = native_loader::create_program_account("bitconch_bpf_loader");
        self.accounts
            .store_slow(true, &bpf_loader::id(), &bpf_loader_account);

        let budget_program_account = native_loader::create_program_account("bitconch_budget_program");
        self.accounts
            .store_slow(true, &budget_program::id(), &budget_program_account);

        let erc20_account = native_loader::create_program_account("bitconch_erc20");
        self.accounts
        ...
        let executed: Vec<Result<()>> = loaded_accounts
            ...
            .map(|(accs, tx)| match accs {
                ...
                Ok((ref mut accounts, ref mut loaders)) => {
                    bitconch_runtime::execute_transaction(tx, loaders, accounts, tick_height).map_err(
...
mod tests {
    ...
    use bitconch_sdk::storage_program::{StorageTransaction, ENTRIES_PER_SEGMENT};
    use bitconch_sdk::system_instruction::SystemInstruction;
    use bitconch_sdk::system_transaction::SystemTransaction;
    use bitconch_sdk::transaction::Instruction;
    use std;
    ...
    // See github issue 1157 (https://github.com/bitconch/bus/issues/1157)
    #[test]
