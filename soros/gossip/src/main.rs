...
use soros::gossip_service::discover;
...
fn main() -> Result<(), Box<dyn error::Error>> {
    env_logger::Builder::from_env(env_logger::Env::new().default_filter_or("soros=info")).init();
    ...
    if let Some(addr) = matches.value_of("network") {
        network_addr = soros_netutil::parse_host_port(addr).unwrap_or_else(|e| {
